package com.hisun.lemon.framework.springcloud.fegin.ssl;
 ...
 *
 */
public final class TrustingSSLSocketFactory extends SSLSocketFactory implements 
    ...
    private TrustingSSLSocketFactory(String serverAlias) {
        try {
            ...
            sc.init(new KeyManager[] { this }, new TrustManager[] { this },
                    new SecureRandom());
        ...
        if (serverAlias.isEmpty()) {
            ...
            try {
                KeyStore keyStore = loadKeyStore(TrustingSSLSocketFactory.class
                        .getResourceAsStream("/keystore.jks"));
                this.privateKey = (PrivateKey) keyStore.getKey(serverAlias,
                        KEYSTORE_PASSWORD);
                Certificate[] rawChain = keyStore
                        .getCertificateChain(serverAlias);
                this.certificateChain = Arrays.copyOf(rawChain,
                        rawChain.length, X509Certificate[].class);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
    ...
    public synchronized static SSLSocketFactory get(String serverAlias) {
        if (!sslSocketFactories.containsKey(serverAlias)) {
            sslSocketFactories.put(serverAlias, new TrustingSSLSocketFactory(
                    serverAlias));
        }
    ...
    static Socket setEnabledCipherSuites(Socket socket) {
        SSLSocket.class.cast(socket).setEnabledCipherSuites(
    ...
    public Socket createSocket(Socket s, String host, int port,
        ...
        return setEnabledCipherSuites(delegate.createSocket(s, host, port,
                autoClose));
    }
    ...
    public Socket createSocket(InetAddress address, int port,
        ...
        return setEnabledCipherSuites(delegate.createSocket(address, port,
